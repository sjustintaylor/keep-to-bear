...
class KeepToBearConverter {
    ...
    async convert() {
        ...
        console.log(`Conversion complete! Converted ${converted} out of ${jsonFiles.length} notes.`);
    ...
    async convertNote(jsonFilePath) {
        ...
        const copiedAttachments = await this.copyAttachments(jsonFilePath, jsonData);

        // Generate markdown
        const markdown = this.generateMarkdown(jsonData, content, copiedAttachments);
    ...
    generateMarkdown(jsonData, content, copiedAttachments = []) {
        ...
        // Add tags from labels - nest under "06 google keep"
        const parentTag = '06-google-keep';
        let allTags = `#${parentTag}`;

        if (jsonData.labels && jsonData.labels.length > 0) {
            const nestedTags = jsonData.labels
                ...
                .map(name => {
                    ...
                    return `#${parentTag}/${processedName.replace(/\s+/g, '-').toLowerCase()}`;
                })
                .join(' ');
...
function main() {
    ...
    if (args.length < 2) {
        console.log('Usage: node keep-to-bear.js <input-directory> <output-directory>');
        console.log('');
        console.log('Example:');
